...
- **`alignment_results/{ovarian_cancer,T_cell_activation}/`** — post-alignment count tables (the inputs to Step 9). Each pair is ~1.2 GB and exceeds GitHub's 100 MB per-file limit, so they are not committed here. Two ways to obtain them:
  - Run `scripts/download_data.{sh,ps1}` to fetch from Dropbox.
  - Click *Reproduce this analysis* on the dashboard's Projects page, which fetches them from this repository's GitHub release on first use.
