#!/usr/bin/env python
"""
TREND ovarian-cancer variant analysis — standalone figure reproduction.
=======================================================================
Reads ONE self-contained data file (trend_ovca_variants.csv) and regenerates
...
Data file — one row per quantified enhancer variant, columns:
  ...
  log2r         log2(ov8 / iose)  = tumor-vs-normal specificity (raw, no pseudocount)

Definitions used throughout:
  consensus variant = the surviving variant of a PWM with the highest rel_affinity
  best variant      = the most TUMOR-specific surviving variant (max log2r),
                      restricted to variants with ov8 >= FLOOR (real tumor activity)
  performance tiers  = all PWMs ranked by their best variant's specificity
"""
...
DATA = HERE / "trend_ovca_variants.csv"
...
for k, g in df.groupby("pwm"):
    ...
    rows.append(dict(pwm=k, tf=cons.tf, nvar=len(g),
        ...
        dT=np.log2(best.ov8/cons.ov8), dN=np.log2(best.iose/cons.iose)))
S = pd.DataFrame(rows)
...
p1 = (ggplot(V, aes("ov8","iose"))
      ...
      + labs(x="OV8 (tumor) RD ratio", y="IOSE (normal) RD ratio",
             title="Per-PWM variant clouds — thick-edged dot = consensus, ★ = most-specific, below diagonal = tumor-specific")
...
a.set_xlabel("Specificity, log2(OV8 / IOSE)"); a.set_ylabel("Number of variants")
...
for ax, (pct, lab) in zip(axs, TIERS):
    ...
    ax.text(.04, .96, f"Median Δtumor {s.dT.median():+.2f}\nMedian Δnormal {s.dN.median():+.2f}", transform=ax.transAxes,
...
p4 = (ggplot(D, aes("x","val", color="cell", group="cell"))
      ...
      + labs(x="", y="Median activity (RD ratio)", color="",
             title="Activity shift consensus → most-specific variant, by performance tier")
...
ax.set_xlabel("Affinity rank of the most tumor-specific variant within its PWM\n(0 = weakest motif match  →  100 = consensus)")
...
ax.set_title("Specificity vs affinity — flat median = affinity doesn't predict specificity")
...
# frontiers: selectivity of the N-th best enhancer each library can assemble (one per PWM)
full = np.sort(S.best_spec.values)[::-1]
cons = np.sort(S.loc[S.cons_ov8 >= FLOOR, "cons_spec"].values)[::-1]
...
axA.set_xlabel("Enhancers selected (ranked by tumor selectivity)")
axA.set_ylabel("Tumor selectivity, OV8 / IOSE")
...
ncons = [int(((S.cons_ov8 >= FLOOR) & (S.cons_spec >= np.log2(t))).sum()) for t in thrs]
...
axB.set_xlabel("Tumor-selectivity threshold"); axB.set_ylabel("Tumor-selective enhancers (PWMs)")
