...
- **`download_data.sh`** (macOS / Linux) and **`download_data.ps1`** (Windows PowerShell) — fetch the ~3 GB of large data files (Lib4 reference, per-construct metadata, full alignment count tables) from the project's Dropbox bundle and place them at the canonical paths the pipeline and R scripts expect. Idempotent: safe to re-run; skips files already in place.
